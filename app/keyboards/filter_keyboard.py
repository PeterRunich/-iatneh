...
async def filter_kb_builder(current_page_number, limit):
    ...
    [kb.insert(InlineKeyboardButton(item[1], callback_data=f"cq1:add_to_filter:{item[0]}")) for item in data] # добавляем жанры в базовый элемент клавиатуры (чтобы работоло ограничение row_width нужно заполнять базовый элемент методом insert)
